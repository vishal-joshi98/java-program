...
int main()
{
	...
	int p[50],priority[50],burst_time[50],execution_time[50],newburst_time[50],newpriority[50],newexecution_time[50],newp[50],total_execution_time[50],at[50],t[50],tt[50],waiting_time[50],rrg[99],tat[50];
    ...
    printf("Adding a new process to the queue\n");                        //new process with higher priority added into the queue
    ...
    printf("Added the lower Priorities to 2nd queue\n");		//First queue execution
    printf("Execution of the first queue\n");
    printf("\nProcess\t    Burst Time    \tWaiting Time\tTurnaround Time");
    for(i=0;i<=0;i++)
    {
        tat[i]=burst_time[i]+waiting_time[i];    
        printf("\nP[%d]\t\t[%d]\t\t%d\t\t%d\t\t\t%d",p[i],burst_time[i],waiting_time[i],tat[i]);
    }
	printf("\nEnter Time Quantum:\n");						//Quantum time
    scanf("%d",&q);
    waiting_time[0]=0;            
    ...
    for(i=0;i<n;i++)
    {
        ...
        printf("\n%d\t\t%d\t\t%d\t\t%d",i+1,total_execution_time[i],waiting_time[i],tt[i]);
    } 
    printf("\n Average Waiting Time:%f",(float)total_waiting_time);
    printf("\n Average Turn Around Time:%f",(float)total_turnaround_time);
    return 0;
}
